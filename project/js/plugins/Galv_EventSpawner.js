...
Game_Map.prototype.initClone = function(){
	if(!this._firstInitedMap.includes(this._mapId)){ // 처음 맵 로딩됨
		let eid = $dataMap.events.length
		for(let x = 0; x < $gameMap.width();x++){
			for(let y = 0; y < $gameMap.height();y++){
				let region = this.regionId(x,y)
				
				if($dataSpawnMap.events[region]){
					this._savedSpawnedEvents[this._mapId][eid] = {id:region,"x":x,"y":y}
					eid++;
				}
			}
		}
		this._firstInitedMap.push(this._mapId)
	}
}
